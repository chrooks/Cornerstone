...
EXPECTED_RAW_COMPOSITES = {
    # Re-pinned 2026-05-25 after composite formula audit:
    # - finishing: crafty_finisher weighted 1.3x (finishing_crafty_weight)
    # - paint_touch: floor 1.0→0.9 (formula convention: max(0.9, 0.9 + scale*finishing))
    # - transition: passer_mult dropped; flat additive transition_passer + transition_off_dribble
    # - ball_security: expanded to passer + pnr_handler + driver (seed lacks new coeffs → 0.0 default)
    # - shot_creation: iso now explicit coefficient (default 1.0 = no change)
    "Stephen Curry": {
        "spacing": 32.0,
        "finishing": 8.0,
        "paint_touch": 6.16,
    ...
    "LeBron James": {
        ...
        "pnr_screener": 0.0,
        "off_ball_impact": 8.8,
        "shot_creation": 42.452,
        "pnr_orchestration": 15.200000000000001,
        "ball_security": 16.0,
    ...
    "Nikola Jokic": {
        "spacing": 8.0,
        "finishing": 8.0,
        "paint_touch": 27.104000000000003,
        "post_game": 13.6,
        "pnr_screener": 16.8,
        "off_ball_impact": 12.8,
        "shot_creation": 27.552,
