...
interface PlayerPanelViewProps {
  ...
  /**
   * "landscape" (default) is the wide, one-per-row scouting card used
   * everywhere panel view already ships (builder picker, admin, /players).
   * "portrait" is the taller-than-wide, fixed-width sheet built for a
   * horizontal-scrolling carousel (Legends picker, #128) — header on top,
   * skills stacked in one column below, mirroring the full profile page.
   */
...
export function PlayerPanelView({
  ...
  const sharedArticleProps = {
    ...
    onDragStart: onDragStart && !disabled ? (event: React.DragEvent) => onDragStart(event, player) : undefined,
